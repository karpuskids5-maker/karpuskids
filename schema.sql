  ...
  for fn in
    ...
    where n.nspname = 'public'
      and p.proname in (
        ...
        'create_payments_snapshot','cleanup_old_login_attempts','mark_messages_read',
...
-- Asegurar columnas que pueden faltar en instancias existentes
...
-- ============================================================
-- RPC: Asignar estudiante a aula
-- Uso: select assign_student_to_classroom(student_id, classroom_id)
-- Para desasignar: select assign_student_to_classroom(student_id, null)
-- ============================================================
...
begin
  ...
  -- Asignar o desasignar (null = quitar del aula)
...
begin
  ...
  select c.id into v_dm_chat_id from public.conversations c
    where c.type = 'direct_message'
      and exists (select 1 from public.conversation_participants where conversation_id = c.id and user_id = NEW.parent_id)
      and exists (select 1 from public.conversation_participants where conversation_id = c.id and user_id = v_teacher_id)
    limit 1;
...
create policy "profiles_select" on public.profiles for select using (
  (auth.uid() = id or get_my_role() in ('directora','asistente','maestra')) and deleted_at is null
);
...
create policy "att_req_parent_insert" on public.attendance_requests for insert
  with check (is_parent_of_student(student_id));
create policy "att_req_parent_select" on public.attendance_requests for select
...
begin
  ...
  select count(*) into v_attendance from public.attendance where date = current_date and status = 'present';
...
begin
  select coalesce(sum(amount),0) into v_debt from public.payments
...
begin
  ...
  ) then raise exception 'No tienes permiso para actualizar este pago.'; end if;
...
begin
  ...
  select coalesce(avg(case when stars is not null then stars::numeric
    when grade_letter='A' then 5 when grade_letter='B' then 4
    when grade_letter='C' then 3 when grade_letter='D' then 2 else null end), 0) into v_task_avg
  ...
  select coalesce(avg(score), 0) into v_formal_avg from public.grades
...
begin
  select due_day into v_due_day from public.school_settings where id = 1;
...
create policy "storage_public_read" on storage.objects for select using (
  ...
  and (auth.uid() = owner or get_my_role() in ('directora','asistente','maestra'))
);
