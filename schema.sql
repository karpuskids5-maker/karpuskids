...
create table if not exists public.grades (
  ...
  student_id   bigint references public.students(id) on delete cascade,
  classroom_id bigint references public.classrooms(id),
  period_id    bigint,  -- FK añadida después de crear periods
  subject      text,
  score        numeric(4,2),
  period       text,
  teacher_id   uuid  references public.profiles(id),
...
-- ¿El usuario es padre de algún estudiante en este aula?
...
-- ¿El usuario es participante de esta conversación? (evita recursión RLS)
  ...
  if new.status = 'pending' then
    ...
    if v_parent_id is not null then
      ...
      values (
        ...
        'Se generó un cargo de $' || new.amount || ' para ' || v_student_name
...
begin
  ...
  -- A. Generar cuotas del próximo mes
...
-- 6.3 Tendencia de asistencia últimos 7 días
...
-- 6.7 Buscar o crear conversación privada
...
-- 6.9 Conteo de mensajes no leídos por remitente
...
-- 6.10 Marcar conversación como leída
...
-- 6.11 Insertar notificación (usada por Edge Functions)
...
-- 6.12 Generar boletín de un estudiante
...
begin
  select * into v_period from public.periods where id = p_period_id;
  if not found then return jsonb_build_object('error','Periodo no encontrado'); end if;
  if v_period.status = 'closed' then return jsonb_build_object('error','Ya está cerrado'); end if;
...
-- 10. DATOS INICIALES
-- ============================================================
-- Configuración escolar (singleton)
insert into public.school_settings (id, generation_day, due_day)
values (1, 25, 5) on conflict (id) do nothing;
