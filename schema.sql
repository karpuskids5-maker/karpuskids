...
-- 9. SISTEMA UNIFICADO DE VIDEOLLAMADAS
-- -----------------------------------------------------------------------------
create table if not exists public.meetings (
  ...
  type text check (type in ('classroom', 'private', 'staff', 'general')), -- classroom=clase, private=tutoría, staff=reunión interna
  target_id text, -- ID del aula, ID del padre, o NULL (general)
...
drop policy if exists "Staff gestiona reuniones" on public.meetings;
create policy "Staff gestiona reuniones" on public.meetings for all using (
  get_my_role() in ('directora', 'maestra', 'asistente')
);

drop policy if exists "Padres ven sus reuniones" on public.meetings;
create policy "Padres ven sus reuniones" on public.meetings for select using (
  (type = 'private' and target_id = auth.uid()::text) or
  (type = 'classroom' and is_parent_of_classroom(target_id::bigint)) or
  (type = 'general')
);
