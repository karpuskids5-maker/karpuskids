...
SELECT v.name, v.education_level, v.description
FROM (VALUES
...
) AS v(name, education_level, description)
...
-- Asegurar 5 areas x 5 actividades para un aula (idempotente)
...
BEGIN
  ...
  -- 5 areas por defecto segun el nivel
  ...
  -- 5 actividades por defecto para cada area
  v_def_acts := ARRAY['Participación en clase','Trabajo individual','Trabajo en equipo','Evaluación escrita','Proyecto o tarea'];

  -- Periodo objetivo: el periodo activo del centro o el ultimo periodo legacy;
  -- si no existe ninguno, se crea uno propio del aula.
  ...
  -- Crear las 5 areas + su period_config + sus 5 actividades (idempotente)
...
-- Trigger: al crear un aula, generar sus areas automaticamente
...
-- Backfill: aulas existentes (idempotente)
DO $$
DECLARE r record;
BEGIN
