...
export const PaymentsModule = {
  ...
  async _getStudentParent(studentId) {
    try {
      const { data } = await supabase.from('students').select('parent_id, parents:parent_id(id, name, phone, email)').eq('id', studentId).maybeSingle();
  ...
  _renderRow(p) {
    ...
    const hasDisc   = discAmt > 0 || discPct > 0;
    ...
    return '<tr class="' + rowClass + '">' +
      ...
      (hasDisc && origAmt > 0 ? '<div class="text-[9px] font-black text-slate-400 line-through">$' + origAmt.toLocaleString('es-DO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + '</div>' : '') +
      '<div class="font-black text-slate-800 text-sm">$' + amountFmt + '</div>' +
      (hasDisc ? '<span class="inline-flex items-center gap-1 px-1.5 py-0.5 mt-0.5 bg-amber-50 border border-amber-200 rounded-full text-[9px] font-black text-amber-700 uppercase" title="Descuento ' + (p.discount_reason || '') + '"><i data-lucide="badge-percent" class="w-2.5 h-2.5"></i> -' + discPct + '%</span>' : '') +
      (moraBlock ? '<div class="mt-0.5">' + moraBlock + '</div>' : '') + receiptCta + reminderCta + '</td>' +
  ...
  _buildActionsCell(id, isPending, statusKey, hasV) {
    ...
    parts.push('<button onclick="App.payments.applyDiscount(\'' + id + '\')" class="p-2 bg-amber-50 text-amber-600 rounded-xl hover:bg-amber-100 border border-amber-100 transition-all active:scale-95" title="Aplicar / quitar descuento"><i data-lucide="badge-percent" class="w-4 h-4"></i></button>');
  ...
  async openPaymentModal(prefillStudentId = null) {
    ...
    const genDayA = Number(this.settings.generation_day) || 25;
    // Mes a cobrar por defecto = mes visible del sistema (regla del día de generación)
    let ty = cy, tm = cm;
    if (now.getDate() < genDayA) { tm = cm === 0 ? 11 : cm - 1; ty = cm === 0 ? cy - 1 : cy; }
    // Vencimiento = día due_day del mes siguiente al mes cobrado.
    const dueMonthM = tm + 1 > 11 ? 0 : tm + 1;
    const dueYearM = tm + 1 > 11 ? ty + 1 : ty;
    const defaultDue = dueYearM + '-' + String(dueMonthM + 1).padStart(2, '0') + '-' + String(this.settings.due_day || 5).padStart(2, '0');
    const monthOpts = MONTH_LABELS.map((lbl, i) => '<option value="' + ty + '-' + String(i + 1).padStart(2, '0') + '"' + (i === tm ? ' selected' : '') + '>' + lbl + '</option>').join('');
    openGlobalModal(
  ...
  async _confirmApproval(id) {
    try {
      // Descuento ANTES de aprobar: ofrecer aplicarlo primero si el pago en
      // revisión aún no tiene descuento (el monto queda fijo al aprobar).
      ...
      if (p && calcStatus(p) === 'review' && !hasDisc && window.confirm('Aplicar un descuento antes de aprobar este pago?\n\nOK = abrir descuento · Cancelar = aprobar ahora mismo')) {
        await this.applyDiscount(id);
        return;
      }
      // Usar el mismo RPC SECURITY DEFINER que el panel de la Directora.
      // Un UPDATE directo vía PostgREST queda sujeto a RLS del rol asistente,
      // que puede devolver éxito sin modificar ninguna fila (aprueba "en falso").
      const { data, error } = await supabase.rpc('approve_payment', {
        p_payment_id: id,
        p_notes: 'Aprobado desde panel de Asistente'
      });
      if (error) throw error;
  ...
  async applyDiscount(id) {
    ...
    openGlobalModal(
      '<div class="bg-gradient-to-r from-amber-500 to-orange-500 text-white p-6 rounded-t-2xl flex items-center gap-3">' +
        ...
        '<div><h3 class="text-xl font-black">Aplicar Descuento</h3><p class="text-xs text-white/70 font-bold uppercase tracking-widest">Mensualidad · ' + Helpers.escapeHTML(p.students?.name || 'Estudiante') + '</p></div>' +
      '</div>' +
      '<div class="p-6 bg-slate-50/30 space-y-4" id="discModalBody">' +
        '<div class="p-4 bg-white rounded-2xl border border-slate-100 grid grid-cols-3 gap-3 text-center">' +
          '<div><div class="text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Monto Original</div><div class="font-black text-slate-700 text-sm" id="discOrig">$' + orig.toLocaleString('es-DO', { minimumFractionDigits: 2 }) + '</div></div>' +
          '<div><div class="text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Desc. Actual</div><div class="font-black text-teal-600 text-sm">' + (curP > 0 ? '-' + curP + '% ' : '') + '$' + saved.toLocaleString('es-DO', { minimumFractionDigits: 2 }) + '</div></div>' +
          '<div><div class="text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Nuevo Total</div><div class="font-black text-emerald-600 text-sm" id="discNew">$' + cur.toLocaleString('es-DO', { minimumFractionDigits: 2 }) + '</div></div>' +
        '</div>' +
        '<div class="grid grid-cols-2 gap-3">' +
        ...
        '<div>' +
          ...
          '<input id="discValue" type="number" step="0.01" min="0" max="100" class="' + inputCls + ' border-2 border-slate-100 focus:ring-4 focus:ring-amber-100 focus:border-amber-400 bg-slate-50/50" value="' + curP + '" placeholder="10">' +
        '</div>' +
        '<div>' +
        ...
        '</div>' +
        '<div class="p-3 bg-teal-50 border border-teal-100 rounded-2xl text-[11px] font-bold text-teal-700 flex items-start gap-2">' +
    ...
    const preview = () => {
      ...
      if (newLabel) newLabel.textContent = '$' + net.toLocaleString('es-DO', { minimumFractionDigits: 2 });
    };
